...
async function objectComposite(prompt, source, mask, id, token) {
...
let result = await objectComposite(prompt, sourceImg, maskImg, CLIENT_ID, token);

for(let i of result.outputs) {
	let fileName = `output/${slugify(prompt)}-${i.seed}-v3.jpg`;
